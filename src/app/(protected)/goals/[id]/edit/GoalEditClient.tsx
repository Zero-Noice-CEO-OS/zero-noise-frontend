...
export default function GoalEdit() {
  ...
  return (
    <div className="space-y-6 max-w-2xl pb-16 animate-fade-in">
      <Link href={`/goals/detail/?id=${id}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-textSecondary hover:text-primary transition-colors">
      ...
      <form onSubmit={handleUpdate} className="bg-surface/50 border border-border rounded-card p-6 shadow-card space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border/40 mt-6">
          <button type="button" onClick={() => router.push(`/goals/detail/?id=${id}`)} className="px-4 py-2 rounded-button bg-surface border border-border text-textPrimary text-xs font-bold hover:bg-background/80 transition-all">Cancel</button>
          <button
            type="submit"
            disabled={updateMutation.isPending}
